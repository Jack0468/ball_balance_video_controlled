...
// Ball position now arrives over serial (see SerialCoords.cpp), not from the
// resistive touchscreen. The name and API are unchanged so nothing else has to
// move; the TouchScreen library is no longer needed.

struct coords {
  double x_mm;
  double y_mm;
  double z;      // >0 = ball present, 0 = lost
};
...
// Drains the serial RX buffer. Optional to call yourself -- coords_available()
// and get_coords() both call it.
void serial_coords_poll();

// True exactly once per received sample. Paces the control loop.
