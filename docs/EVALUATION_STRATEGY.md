...
**Correction/update (2026-09-24): the real target is <3mm, not <10mm, and the ~4-5mm vision RMSE figure above is now known to be understated.** A 2026-09-23/24 investigation (see `docs/PROJECT_LOGBOOK.md`'s "23-24/09/2026 (overnight)" entry) measured vision's own accuracy against touch ground truth directly, across 40,713 real frames from 10 recorded sessions: mean error 5.6-6.4mm depending on outlier trim/methodology, not 4-5mm — a real, systematic, position-dependent bias, not just noise. The control loop closes on vision (never touch — see `BallBalancingBot.ino`'s own comment), so this bias propagates close to directly into Steady-State Error, which is why current measured values (~9-17mm) sit well above the stated 10mm bar. A per-session calibration correction has been designed and offline-validated (5.6mm → 4.4-4.5mm, ~20% reduction) but is **NOT yet integrated or hardware-tested** — see `host_software/ml_jetson_vla/docs/VISION_CALIBRATION_PROPOSAL.md` for the full analysis, validated numbers, and what's still required before it touches real hardware. Closing the remaining gap to <3mm will need an additional, not-yet-investigated lever (most likely control-loop/settling-dynamics tuning).
