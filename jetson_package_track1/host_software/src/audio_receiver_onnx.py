    ...
    def _process_loop(self):
        ...
        while self.running:
            ...
            if aligned is None:
                ...
                # --- DEBUG PRINT ---
                print(
                    f"[AUDIO DEBUG] Pred: {top_label} | Conf: {top_conf:.2f} | Margin: {margin:.2f}"
                )

                if top_conf >= self.min_confidence and margin >= self.min_margin:
                    if (
                        top_label != "_background_"
                        and top_label != self.last_pushed_command
                    ):
