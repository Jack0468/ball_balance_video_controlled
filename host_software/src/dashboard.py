...
def _dashboard_worker(queue):
    ...
    while True:
        ...
        for name, pos in state["marker_coords"].items():
            ...
            ax_plot.text(
                mx, my + 4, name, color=color, fontsize=6, ha="center", va="bottom"
            )
        ax_plot.plot(
            state["target_x"],
            state["target_y"],
...
class LiveDashboard:
    ...
    def update(
        ...
        payload = {
            ...
            "marker_coords": {
                k: (float(v[0]), float(v[1])) for k, v in marker_coords.items()
            },
            "command": command,
            "fps": float(fps),
