...
def create_printable_sheet(output_path="aruco_markers_sheet.png", marker_size_px=400):
    ...
    for i in range(4):
        ...
        cv2.putText(
            marker_bgr, labels[i], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 2
        )

        cv2.imwrite(
            os.path.join(os.path.dirname(output_path), f"marker_{i}.png"), marker_bgr
        )
    ...
    bottom_row = np.hstack(
        (markers[3], markers[2])
    )  # 3 is Bottom-Left, 2 is Bottom-Right
    ...
    print(
        "2. Measure the printed black squares to ensure they match your expected physical size (e.g., 30mm x 30mm)."
    )
    print("3. Cut them out leaving a small white border.")
    print(
        "4. Tape them to the 4 corners of your physical platform according to the labels."
    )
