...
TARGET_COORDS = {
    ...
    "yellow": (
        -93.75 + 69,
        -71.0 + 58,
    ),  # mapped to grey marker: 69 from left, 58 from bottom
    "blue": (
        93.75 - 13,
        -71.0 + 8,
    ),  # mapped to black marker: 13 from right, 8 from bottom
}
# ---------------------
...
def main():
    ...
    try:
        while True:
            ...
            cv2.putText(
                frame,
                f"Target ({target_colour}): X={target_x:.1f} Y={target_y:.1f}",
                (20, 60),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (0, 165, 255),
