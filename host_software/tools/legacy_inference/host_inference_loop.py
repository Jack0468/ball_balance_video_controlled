...
def main():
    ...
    parser.add_argument(
        ...
        choices=["udp", "usb"],
        default="usb",
        help="Camera source (udp or usb)",
    )
    parser.add_argument(
        "--cam_id", type=int, default=0, help="Camera ID for USB mode (0, 1, 2, etc)"
    )
    ...
    model_path = os.path.abspath(
        os.path.join(
            script_dir, "models/resnet18_expert_tracker_iphone_v1/expert_tracker_best.pth"
        )
    )
    ...
    try:
        while True:
            ...
            cv2.putText(
                frame,
                f"Pred: X={touch_x:.1f} Y={touch_y:.1f} mm",
                (20, 50),
