...
def main():
    ...
    model_path = os.path.join(
        script_dir, "models/yolov8_unified_pose_v1/weights/best.pt"
    )
    if not os.path.exists(model_path):
        print(
            f"ERROR: Unified YOLO-Pose model not found at {model_path}. Train the model first."
        )
        return
            ...
            if platform_kpts is not None:
                ...
                if M is not None:
                    ...
                    if ball_px is not None:
                        # Project ball to MM
                        b_px_arr = np.array(
                            [[[ball_px[0], ball_px[1]]]], dtype=np.float32
                        )
                        ...
                        cv2.putText(
                            annotated_frame,
                            f"Cam: X={final_x:.1f} Y={final_y:.1f} mm",
                            (20, 90),
                    ...
                    for cls_id, t_px in target_pxs:
                        ...
                        cv2.putText(
                            annotated_frame,
                            f"T{cls_id}: {tx:.0f},{ty:.0f}mm",
                            (int(t_px[0]), int(t_px[1] - 10)),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (255, 255, 255),
                            2,
