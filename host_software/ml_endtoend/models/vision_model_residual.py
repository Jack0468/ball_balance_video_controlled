...
# ---------------------------------------------------------------------------
# 1. The MLP Corrector Architecture (Residual Network)
# ---------------------------------------------------------------------------
class ResBlock(nn.Module):
    """
    A standard Residual Block for MLPs. 
    Matches the saved state_dict indices (0: Linear, 1: Activation, 2: Dropout, 3: Linear).
    """
    def __init__(self, dim=128):
        super(ResBlock, self).__init__()
        self.block = nn.Sequential(
            nn.Linear(dim, dim),      # index 0: weight & bias
            nn.ReLU(),                # index 1: weightless
            nn.Dropout(0.1),          # index 2: weightless
            nn.Linear(dim, dim)       # index 3: weight & bias
        )
...
class CorrectorMLP(nn.Module):
    ...
    (cam_x, cam_y) coordinates using a Residual architecture.
    """
    def __init__(self, input_dim=14, hidden_dim=128, output_dim=2):
        super(CorrectorMLP, self).__init__()
        
        # Matches: "input_layer.0.weight", "input_layer.0.bias"
        self.input_layer = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU()
        )
        
        # Matches: "res_blocks.0.block...", "res_blocks.1.block..."
        self.res_blocks = nn.Sequential(
            ResBlock(hidden_dim),
            ResBlock(hidden_dim)
        )
        
        # Matches: "output_layer.weight", "output_layer.bias"
...
def load_mlp_corrector_v1_model(model_path, device):
    ...
    if os.path.exists(model_path):
        # Strict loading ensures we crash cleanly if the shapes don't match
