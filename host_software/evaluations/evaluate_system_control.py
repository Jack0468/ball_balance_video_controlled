...
def evaluate_telemetry(csv_path, output_dir):
    ...
    df = df.dropna(subset=['target_x', 'target_y', 'touch_x', 'touch_y', 'theta_a', 'theta_b', 'theta_c'])
    
    # 1. Steady-State Error
    # Calculate euclidean error for every frame
    errors = np.sqrt((df['touch_x'] - df['target_x'])**2 + (df['touch_y'] - df['target_y'])**2)
    ...
    plt.plot(plot_df['host_timestamp_ms'] - plot_df['host_timestamp_ms'].iloc[0], plot_df['target_x'], 'r--', label='Target X')
    plt.plot(plot_df['host_timestamp_ms'] - plot_df['host_timestamp_ms'].iloc[0], plot_df['touch_x'], 'r-', label='Ball X')
    plt.plot(plot_df['host_timestamp_ms'] - plot_df['host_timestamp_ms'].iloc[0], plot_df['target_y'], 'b--', label='Target Y')
    plt.plot(plot_df['host_timestamp_ms'] - plot_df['host_timestamp_ms'].iloc[0], plot_df['touch_y'], 'b-', label='Ball Y')
    plt.xlabel("Time (ms)")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_path', type=str, required=True, help="Path to telemetry CSV")
    parser.add_argument('--output_dir', type=str, default="results", help="Directory to save metrics")
