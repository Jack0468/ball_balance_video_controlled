"""EXPERIMENTAL ENTRY POINT: YOLOv8 with Platform Markers. Variant that attempted to track both the ball and physical platform markers for pose estimation."""
...
def main():
    ...
    parser.add_argument(
        "--head",
        action="store_true",
        help="Show video feed with detections for debugging",
    )
    parser.add_argument(
        "--mock", action="store_true", help="Use mock receiver with 0000.jpg"
    )
    ...
    audio_xml = os.path.abspath(
        os.path.join(
            script_dir, "ml_audio/models/audio_command_classifier/best_classifier.xml"
        )
    )
    ...
    if args.mock:
        ...
        receiver = (
            UDPReceiver(port=args.udp_port, width=640, height=480)
            if args.udp
            else USBReceiver(camera_id=args.cam_id)
        )
    ...
    try:
        while True:
                    ...
                    if res.boxes is not None:
                        ...
                        if corners is not None and ball_box is not None:
                            ...
                            if projector.update_homography(corners):
                                hx, hy = projector.project_point(
                                    ball_box[0], ball_box[1]
                                )
                            ...
                            kpts_shapes = [
                                (
                                    res.keypoints.xy[j].cpu().numpy().shape
                                    if res.keypoints is not None
                                    and len(res.keypoints.xy) > j
                                    else None
                                )
            ...
            if cam_coords is not None:
                ...
                print(
                    f"Targeting '{state_machine.current_target_name}' at X={target_x:.1f} Y={target_y:.1f} | Ball: X={cam_x:.1f} Y={cam_y:.1f} mm | FPS: {fps:.1f}"
                )
