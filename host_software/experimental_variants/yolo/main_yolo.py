"""EXPERIMENTAL ENTRY POINT: YOLOv8 Baseline. Early vision pipeline relying solely on YOLOv8 for ball tracking."""
        ...
        while True:
            ...
            if frame is not None:
                ...
                padded_frame = cv2.copyMakeBorder(
                    rgb_frame, 80, 80, 0, 0, cv2.BORDER_CONSTANT, value=(114, 114, 114)
                )

                # YOLOv8 input is NCHW (1, 3, 640, 640), RGB, scaled to 0-1
                img = (
                    padded_frame.transpose((2, 0, 1))[np.newaxis, ...].astype(
                        np.float32
                    )
                    / 255.0
                )
            ...
            if yolo_res is not None:
                ...
                if len(yolo_res.shape) == 2:
                    ...
                    class_scores = np.max(
                        boxes_transposed[:, 4 : 4 + num_classes], axis=1
                    )
                    class_ids = np.argmax(
                        boxes_transposed[:, 4 : 4 + num_classes], axis=1
                    )
