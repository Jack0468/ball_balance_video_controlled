"""EXPERIMENTAL ENTRY POINT: Host-side PID Control Loop. Used for early testing before the PID loop was offloaded to the STM32/FPGA."""
...
def main():
    ...
    try:
        while True:
            ...
            # We send cam_x, cam_y, target_x, target_y so the PID firmware gets true absolute coords
            ...
            try:
                ...
                if ser is not None:
                    ser.write(payload)
                    # Flush the input buffer so any strings/telemetry from the PID firmware don't overflow
