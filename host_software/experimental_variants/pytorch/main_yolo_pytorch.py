"""EXPERIMENTAL ENTRY POINT: YOLOv8 via PyTorch. Alternative implementation of YOLO inference without ONNX acceleration."""
    ...
    try:
        while True:
            ...
            if corners is None or ball_box is None:
                ...
                print(
                    f"Missing detections - Platform: {'Found' if corners is not None else 'Missing'}, Ball: {'Found' if ball_box is not None else 'Missing'} | FPS: {fps:.1f} | YOLO Latency: {yolo_latency_ms:.1f}ms"
                )
                continue
            ...
            print(
                f"Target: X={cam_x:.1f} Y={cam_y:.1f} mm | FPS: {fps:.1f} | Latency: Total={total_latency_ms:.1f}ms (YOLO={yolo_latency_ms:.1f}ms, MLP+Rest={mlp_latency_ms:.1f}ms)"
            )
