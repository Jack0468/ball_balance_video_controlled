...
class VLADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # State Token
        state = torch.tensor([item.get('state_x', 0), item.get('state_y', 0)], dtype=torch.float32)
        
        # Action Targets (Expert PID output to imitate)
        action = torch.tensor([item.get('action_theta_a', 0), item.get('action_theta_b', 0), item.get('action_theta_c', 0)], dtype=torch.float32)
