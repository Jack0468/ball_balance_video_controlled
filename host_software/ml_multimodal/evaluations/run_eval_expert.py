...
def main():
    ...
    audio_model_path = os.path.abspath(
        os.path.join(
            script_dir,
            "ml_audio/synthetic/models/pytorch/audio_weights_with_synthetic.pth",
        )
    )
    master_audio_path = os.path.abspath(
        os.path.join(
            script_dir, "ml_audio", "data", "02_silver", "master_evaluation_audio.wav"
        )
    )
    ...
    if not os.path.exists(master_audio_path):
        ...
        print(
            "Please run `python ml_audio/create_master_audio.py` first to generate the streaming sequence."
        )
        return
    ...
    try:
        ...
        print(
            f"Serial port {args.port} unavailable. Evaluation requires physical hardware!"
        )
        return
    ...
    try:
        ...
        audio_receiver = AudioCommandReceiver(
            audio_model_path, source_file=master_audio_path
        )
            ...
            while ser.in_waiting > 0:
                ...
                if bytes(sync_buf) == b"\xaa\xbb\xcc\xdd":
                    ...
                    if len(data) == expected_size - 4:
                        ...
                        csv_writer.writerow(
                            [host_command_sent_ms, host_packet_received_ms]
                            + list(unpacked)
                        )
                    sync_buf.clear()
