...
class TrainingDataCollector:
    def __init__(self, teensy_port):
        ...
        self.bronze_dir = os.path.join(
            os.path.dirname(__file__), "..", "..", "ml_vision", "data", "bronze"
        )
                ...
                while True:
                    ...
                    if bytes_read == self.payload_size:
                        if buf[0:4] != self.sync_header:
                            print(
                                "Warning: Sync header mismatch on FPGA read. Waiting for next frame."
                            )
                            time.sleep(0.01)
                            continue
                        ...
                        filename = (
                            f"frame_{host_timestamp_ms}_{self.frames_saved:05d}.png"
                        )
                        ...
                        print(
                            f"Warning: Dropped payload. Expected {self.payload_size} bytes, got {bytes_read}."
                        )
                        time.sleep(0.05)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Teensy COM port (e.g. COM3 or /dev/ttyACM0)",
    )
