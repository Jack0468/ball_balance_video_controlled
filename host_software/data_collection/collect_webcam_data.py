...
class WebcamDataCollector:
    def __init__(self, port, baudrate=2000000, cam_id=0, width=640, height=480, fps=30.0):
        self.port = port
        ...
        self.frame_timestamps_path = os.path.join(self.session_dir, 'frame_timestamps.csv')
    ...
    def _open_telemetry_file(self):
        ...
        if os.path.getsize(self.telemetry_path) == 0:
            self.telemetry_writer.writerow([
                'host_timestamp_ms', 'mcu_micros', 'target_x', 'target_y',
    ...
    def _open_frame_timestamps_file(self):
        self.frame_timestamps_file = open(self.frame_timestamps_path, mode='a', newline='')
        ...
        try:
            while True:
                ...
                if self.is_recording:
                    ...
                    self.frame_timestamps_writer.writerow([self.recorded_frame_index, timestamp_ms])
                    self.frame_timestamps_file.flush()
                    self.recorded_frame_index += 1
                    cv2.putText(display_frame, f'RECORDING ({self.recorded_frame_index})', (20, 40),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
                else:
                    cv2.putText(display_frame, 'READY (press r to record)', (20, 40),
                ...
                if current_tel:
                    cv2.putText(display_frame, f'TX: {current_tel[2]:.1f} TY: {current_tel[3]:.1f}', (20, 80),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                    cv2.putText(display_frame, f'Ball: {current_tel[4]:.1f}, {current_tel[5]:.1f}', (20, 120),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                else:
                    cv2.putText(display_frame, 'WAITING FOR TELEMETRY...', (20, 80),
            ...
            print(f'Output files: {self.video_path}, {self.telemetry_path}, {self.frame_timestamps_path}')


def main():
    parser = argparse.ArgumentParser(description='Collect webcam video and STM32 telemetry for offline sync.')
    parser.add_argument('--port', type=str, default='auto', help='STM32 COM port (e.g. COM3 or auto)')
    ...
    collector = WebcamDataCollector(port, args.baud, args.cam, args.width, args.height, args.fps)
    collector.run()
