...
## Status
In progress. Corruption audit + quarantine, evaluation-script extraction, and the live receiver's label-order + preprocessing fixes are all done and measured. The retrain track now has a working end-to-end pipeline ([`training/train_audio_command_classifier.py`](../../training/train_audio_command_classifier.py)) and two real trained checkpoints: **v4** (quarantine-cleaned dataset only: 83.5% offline, 6/11 live-stream, up from v3's 4/11) and **v5** (v4 plus ~1057 real-noise background clips segmented from the 23-minute lab recording: 84.9% offline and 88.9% background recall — both better than v4 — but 5/11 live-stream, a regression from v4 on the metric that matters most). Net result so far: retraining clearly helps over the original v3 baseline, but background diversification's effect on the live-stream number specifically is not yet a clean win and needs a tuning/multi-seed pass before treating either v4 or v5 as final — see the results tables below. Building the training script also surfaced an important correction to an assumption made earlier in this same plan: see "Correction: the conv/batchnorm layers are NOT frozen/shared across checkpoints" below. Companion to [`.agents/agent_ml_audio.md`](../../../../.agents/agent_ml_audio.md).
...
**Not yet done: running the actual Colab sweep** (needs a human to open the notebook, mount Drive/upload the package, and execute it — not something run from here) and evaluating the winner locally with both `evaluate_audio_classifier.py` and `evaluate_live_receiver_stream.py`. Given v5's offline-vs-live-stream divergence, the live-stream number is the one that decides whether this tuning pass actually closes the gap, not the sweep's own val accuracy.
