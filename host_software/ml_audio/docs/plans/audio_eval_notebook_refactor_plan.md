...
## Data & Checkpoint Location -- NOT in Git (read this before starting on another machine)

Verified directly (`git ls-files`), not assumed: **the actual training dataset and every trained checkpoint referenced in this plan are 0% tracked in git.** `host_software/ml_audio/data/` and `models/`/`*.pth`/`*.onnx` are all gitignored. A fresh clone on another device gets every script, notebook, and this plan doc's full narrative/numbers -- but none of the ~20K-clip `data/synthetic+real_dataset_large/training_v2/` dataset, and none of the `models/nemo_matchboxnet_v1*/` or `models/pytorch_v4`-`v7` checkpoints. Those currently exist only on this machine, plus whatever the Colab notebooks have pushed to Google Drive (`DRIVE_OUTPUT_DIR` / `ml_audio_colab_package.zip` in `colab_nemo_finetune.ipynb` and the augmentation-sweep notebook) -- **Drive, not git, is the de facto source of truth for data + checkpoints right now**, which was never a deliberate decision, just where things ended up.

**Open item, not yet resolved:** `updated dataset.zip` (repo root of `ml_audio/`, git-tracked, committed 2026-07-24 -- before this plan's work started) turned out on inspection to be raw source recordings for `speaker02` (`updated dataset/speaker02/go_blue/speaker02__go_blue__001.wav`, etc.), almost certainly the raw material behind the `real_speaker02__*` clips already baked into `training_v2/`. This is plausibly a partial answer to the "second lab partner's unlocated dataset-generation source" discrepancy noted below -- worth investigating properly (a dedicated data-management pass), not deleting on a first impression that it was stale clutter, which it isn't.
