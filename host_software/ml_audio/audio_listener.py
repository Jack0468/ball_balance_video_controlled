...
class AudioListener:
    ...
    def _inference_loop(self):
        while self.running:
            ...
            # Predict
            audio_aligned, info = align_speech_to_fixed_length(
                waveform, target_samples=OUTPUT_SEQUENCE_LENGTH
            )
            if audio_aligned is None:
                continue  # Too quiet or no speech

            waveform_tf = tf.convert_to_tensor(audio_aligned, dtype=tf.float32)[
                tf.newaxis, :
            ]
