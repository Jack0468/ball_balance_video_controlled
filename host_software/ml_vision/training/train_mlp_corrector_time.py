...
class TimeWindowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features = input_block[['cnn_pixel_x', 'cnn_pixel_y', 'target_x', 'target_y', 'dt']].values.astype(np.float32)
        
        # Normalize features to roughly [-1, 1]
        features[:, 0] = (features[:, 0] / 320.0) - 1.0 # cnn_pixel_x: [0, 640] -> [-1, 1]
        features[:, 1] = (features[:, 1] / 240.0) - 1.0 # cnn_pixel_y: [0, 480] -> [-1, 1]
...
def main():
    ...
    parser.add_argument("--future_offset", type=int, default=0, help="0=predict current, 1=predict next frame (~33ms future)")
    ...
    save_dir = os.path.abspath(os.path.join(script_dir, f'../models/mlp_corrector_time_v{version_num}'))
    ...
    dataset = TimeWindowDataset(csv_path, window_size=args.window_size, future_offset=args.future_offset)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            print(f"Epoch {epoch+1}/{num_epochs} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Best: {best_loss:.4f}")
