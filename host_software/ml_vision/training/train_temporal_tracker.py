...
from temporal_ball_dataset import (
    SequenceBallDataset,
    TemporalTrainTransform,
    TemporalTestTransform,
)
import argparse
...
def train_model():
    ...
    parser.add_argument(
        "--save_dir",
        default="../models/resnet18_temporal_tracker_v1",
    ...
    full_dataset_train = SequenceBallDataset(
        csv_file=csv_path,
        root_dir=images_dir,
        seq_len=seq_len,
        transform=train_transform,
    )
    full_dataset_test = SequenceBallDataset(
        csv_file=csv_path,
        root_dir=images_dir,
        seq_len=seq_len,
    ...
    print(
        f"Total Sequences: {len(full_dataset_train)}. Train: {len(train_dataset)} | Test: {len(test_dataset)}"
    )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        checkpoint = {
            ...
            "best_test_loss": (
                best_test_loss if avg_test_loss >= best_test_loss else avg_test_loss
            ),
        }
