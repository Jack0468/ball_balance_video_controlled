...
def unified_loss(preds, targets):
    ...
    # Combined Loss
    return loss_coords + (0.1 * loss_presence) # Weight presence slightly less to prioritize accuracy of coordinates
...
def main():
    ...
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=4, pin_memory=(device.type == 'cuda'))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        save_path = os.path.join(project_dir, 'cnn_unified_tracker_v1/unified_tracker_best.pth')
        if epoch_test_loss < best_loss:
            best_loss = epoch_test_loss
            torch.save(checkpoint, save_path)
            print(f"Saved new best model to {save_path}")
            
        latest_path = os.path.join(project_dir, 'cnn_unified_tracker_v1/unified_tracker_latest.pth')
