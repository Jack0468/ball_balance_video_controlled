...
class UnifiedYoloFeatureDataset(Dataset):
    def __init__(self, df):
        ...
        self.X[:, 0::2] = np.where(self.X[:, 0::2] >= 0, self.X[:, 0::2] / 640.0, -1.0) # x coords
        self.X[:, 1::2] = np.where(self.X[:, 1::2] >= 0, self.X[:, 1::2] / 480.0, -1.0) # y coords
...
def train():
    ...
    parser.add_argument("--data_csv", default="../../data/02_silver/session_20260728_102908/yolo_features.csv", help="Path to yolo_features.csv")
    ...
    save_dir = os.path.abspath(os.path.join(script_dir, '../models/unified_corrector_v1'))
