...
- [ ] **Custom Model Fine-Tuning**: Collect a high-quality dataset of the specific ball rolling on the final platform. Label the data and use it to fine-tune our ResNet18 expert model. *Note for future: the ResNet18 backbone was chosen so we can easily add multi-task heads later. Eventually, we want to predict the location of the coloured markers, and possibly directly predict control signals ("go to red marker") from the same shared base model. Additionally, we plan to eventually synthesize and encode the trained model weights directly onto the FPGA to perform hardware-accelerated inference without the host PC!*
