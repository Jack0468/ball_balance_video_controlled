...
class BallDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = torch.tensor(
            [touch_x / MAX_BOUND, touch_y / MAX_BOUND], dtype=torch.float32
        )
...
def find_resnet_checkpoint(model_root: str) -> Optional[str]:
    for name in [
        "expert_tracker_best.pth",
        "expert_tracker_latest.pth",
        "model_best.pth",
        "best.pth",
    ]:
...
def find_mlp_checkpoint(model_root: str) -> Optional[str]:
    ...
    pths = [
        os.path.join(model_root, fn)
        for fn in os.listdir(model_root)
        if fn.endswith(".pth") or fn.endswith(".bin")
    ]
    return pths[0] if pths else None
...
def summarize_timing(times: List[float]) -> Dict[str, float]:
    ...
    return {
        ...
        "FPS_Estimate": (
            float(1000.0 / np.mean(times_arr)) if np.mean(times_arr) > 0 else 0.0
        ),
...
def main():
    parser = argparse.ArgumentParser(
        description="Quick inference benchmark for YOLO, ResNet, and MLP models"
    )
    parser.add_argument(
        "--models_dir",
    ...
    parser.add_argument(
        "--model-types",
        type=str,
        default="yolo,resnet,mlp,cnn",
        help="Comma-separated model types to benchmark: yolo,resnet,mlp,cnn",
    )
    parser.add_argument(
        "--model-names",
        type=str,
    ...
    for model_name in sorted(os.listdir(args.models_dir)):
        ...
        try:
            if "yolo" in model_name.lower() and "yolo" in allowed_model_types:
                ...
                if model_path is None:
                    ...
                    continue
                sample_df = select_sample_rows(
                    df, args.sample_fraction, args.max_samples, args.seed
                )
                metrics = benchmark_yolo_model(
                    model_name, model_path, sample_df, images_dir
                )
            elif ("resnet" in model_name.lower() or "cnn" in model_name.lower()) and (
                "resnet" in allowed_model_types or "cnn" in allowed_model_types
            ):
                ...
                test_dataset = BallDataset(
                    os.path.join(args.data_dir, "labels_sequential.csv"),
                    images_dir,
                    transform=transforms.Compose(
                        [
                            (
                                ...
                                if (
                                    "resnet18" in model_name.lower()
                                    or "cnn" in model_name.lower()
                                )
                                else transforms.Resize((480, 640))
                            ),
                            transforms.ToTensor(),
                ...
                sample_indices = sample_dataset_indices(
                    len(test_dataset), args.sample_fraction, args.max_samples, args.seed
                )
                metrics = benchmark_resnet_model(
                    model_name, checkpoint_path, sample_indices, test_dataset, device
                )
                ...
                yolo_model_path = os.path.join(
                    args.models_dir,
                    "yolov8_platform_pose_markers_v1",
                    "weights",
                    "best.pt",
                )
                if not os.path.exists(yolo_model_path):
                    raise FileNotFoundError(
                        f"YOLO model not found at {yolo_model_path}. Please ensure yolov8_platform_pose_markers_v1/weights/best.pt exists."
                    )
                sample_df = select_sample_rows(
                    df, args.sample_fraction, args.max_samples, args.seed
                )
                metrics = benchmark_mlp_model(
                    model_name, yolo_model_path, mlp_path, sample_df, images_dir, device
                )
            else:
                continue
        ...
        print(
            f"{model_name}: {metrics['Mean_Inference_Time_ms']:.2f} ms avg over {metrics['Sampled_Frames']} frames"
        )
