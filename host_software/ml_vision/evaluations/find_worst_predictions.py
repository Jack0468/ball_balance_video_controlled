...
def find_worst_predictions(model_path, data_dir, output_path):
    ...
    with torch.no_grad():
        ...
        for inputs, targets in test_loader:
            ...
            out_np = outputs.cpu().numpy() * np.array([MAX_X_BOUND, MAX_Y_BOUND])
            targ_np = targets.cpu().numpy() * np.array([MAX_X_BOUND, MAX_Y_BOUND])
    ...
    for idx, item in enumerate(worst_16):
        ...
        if os.path.exists(img_path):
            ...
            if img is not None:
                ...
                ax.imshow(img)
                ax.set_title(
                    f"Err: {item['error']:.1f}mm | {item['inference_time_ms']:.1f}ms\nTarget: ({item['targ_x']:.1f}, {item['targ_y']:.1f})"
                )
        else:
