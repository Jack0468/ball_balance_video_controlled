...
def main():
    parser = argparse.ArgumentParser(
        description="Quick Visual Evaluation for Partially Trained Model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_images", type=int, default=6, help="Number of random images to visualize"
    )
    ...
    print(
        f"Inference completed in {(t1-t0)*1000.0:.2f} ms for {args.num_images} images."
    )
    ...
    fig.suptitle(
        "Quick Visual Evaluation (Partially Trained Model)\nGreen = Ground Truth | Red = Prediction",
        fontsize=16,
    )
    ...
    fig.legend(
        by_label.values(), by_label.keys(), loc="lower center", ncol=2, fontsize=12
    )
