...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Custom CNN 2D Expert Tracker"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_path",
        required=True,
        help="Path to the trained .pth file (e.g. models/cnn_2d_tracker/cnn_2d_tracker_v1/expert_tracker_best.pth)",
    )
    ...
    for k, v in metrics.items():
        if "R2" in k:
            ...
            print(
                f"{k}: {v:.2f} mm"
                if "Time" not in k and "FPS" not in k and "R2" not in k
                else f"{k}: {v:.2f}"
            )
    ...
    if (
        r2_x < 0.1
        or r2_y < 0.1
        or metrics["Pred_StdDev_X_mm"] < 5.0
        or metrics["Pred_StdDev_Y_mm"] < 5.0
    ):
        print("\n[WARNING] MODE COLLAPSE DETECTED!")
        print(
            "The R2 Score is ~0 (or negative) AND/OR the Prediction StdDev is very low."
        )
        print(
            "This means the neural network is just outputting the center of the board constantly, instead of tracking the ball."
        )
