...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to cnn_sequential_features.csv",
    )
    parser.add_argument(
    ...
    full_dataset = TimeWindowDataset(
        csv_file=csv_path,
    ...
    plt.subplot(2, 1, 1)
    plt.plot(
        targs[:plot_len, 0], label="Actual X", color="blue", alpha=0.7, linewidth=2
    )
    plt.plot(
        preds[:plot_len, 0],
        label="Predicted X",
    ...
    plt.subplot(2, 1, 2)
    plt.plot(
        targs[:plot_len, 1], label="Actual Y", color="blue", alpha=0.7, linewidth=2
    )
    plt.plot(
        preds[:plot_len, 1],
        label="Predicted Y",
