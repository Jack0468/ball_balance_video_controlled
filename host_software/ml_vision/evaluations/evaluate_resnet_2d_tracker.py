...
def main():
    ...
    parser.add_argument(
        "--data_dir",
        default="../../data/02_silver/cropped_yolo",
        help="Path to cropped data directory",
    )
    parser.add_argument(
        "--csv_name",
    ...
    if has_sequential_fc:
        ...
        model.fc = nn.Sequential(nn.Dropout(p=0.5), nn.Linear(num_ftrs, 2))
    else:
    ...
    for k, v in metrics.items():
        print(
            f"{k}: {v:.2f} mm"
            if "Time" not in k and "FPS" not in k
            else f"{k}: {v:.2f}"
        )
