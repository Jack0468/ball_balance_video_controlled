...
def main():
    parser = argparse.ArgumentParser(description="Verify Dataset Labels via Video Generation")
    parser.add_argument("--data_dir", required=True, help="Path to data directory (e.g. host_software/data/02_silver/session_20260730_174916)")
    parser.add_argument("--csv_name", default="yolo_features.csv", help="Name of the CSV labels file")
    parser.add_argument("--output_name", default="validation_labels.mp4", help="Name of the output video file")
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if not pd.isna(ball_x) and not pd.isna(ball_y):
            ...
            cv2.circle(frame, (int(ball_x), int(ball_y)), 8, (255, 255, 255), 2) # White outline
            
            # Put text for coordinate reference
            cv2.putText(frame, f"Label: ({int(ball_x)}, {int(ball_y)})", (10, 30), 
