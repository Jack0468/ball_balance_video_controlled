...
def analyze_log(filename):
    ...
    def find_peaks(err_list):
        ...
        for i in range(1, len(err_list) - 1):
            if abs(err_list[i]) > abs(err_list[i - 1]) and abs(err_list[i]) > abs(
                err_list[i + 1]
            ):
    ...
    print(
        f"Y peaks (mm): {[round(p, 1) for p in y_peaks[:10]]}... (total {len(y_peaks)})"
    )
    print(
        f"X peaks (mm): {[round(p, 1) for p in x_peaks[:10]]}... (total {len(x_peaks)})"
    )
...
def live_plot(port):
    ...
    ani = animation.FuncAnimation(
        fig, update, interval=33, blit=True, cache_frame_data=False
    )
    plt.tight_layout()
    plt.show()
    ser.close()
