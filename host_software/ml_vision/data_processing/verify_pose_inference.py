...
def main():
    ...
    parser.add_argument("--session_dir", required=True, help="Path to session dir (e.g. host_software/data/02_silver/session_20260728_102908)")
    ...
    if not os.path.exists(csv_path):
        print(f"Error: {csv_path} not found. Run auto_label_telemetry_with_pose.py first.")
        return
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Writing frames"):
        ...
        cv2.putText(img, "Projected Ball", (bx1, by1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
