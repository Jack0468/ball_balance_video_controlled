...
def main():
    ...
    if not dataset_root:
        raise ValueError(
            "The raw_dataset.yaml must contain a valid path field pointing to the dataset root."
        )
    if not os.path.isabs(dataset_root):
        dataset_root = os.path.abspath(
            os.path.join(script_dir, "..", "..", "..", dataset_root)
        )
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
        ...
        for source in sorted(sources):
            ...
            print(
                f"Adding {len(to_add)} labeled files from {images_rel} (labels: {len(labeled_imgs)}, backgrounds excluded: {len(background_imgs)})"
            )
