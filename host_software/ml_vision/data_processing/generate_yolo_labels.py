...
def main():
    parser = argparse.ArgumentParser(
        description="Generate YOLO bounding box labels using Homography"
    )
    parser.add_argument(
        "--video", required=True, help="Path to input video (e.g., in 02_silver)"
    )
    parser.add_argument(
        "--telemetry", required=True, help="Path to the synced telemetry CSV"
    )
    parser.add_argument(
        "--output_labels", required=True, help="Directory to save YOLO .txt labels"
    )
    parser.add_argument(
        "--output_images", required=True, help="Directory to save training images"
    )
    ...
    print(
        "Please click the 4 markers in order: Green (TL), Red (TR), Black (BR), Grey (BL)"
    )
    ...
    while True:
        ...
        for mx, my in clicked_points:
            ...
            mh_norm = 20.0 / h
            label_lines.append(
                f"1 {mx_norm:.6f} {my_norm:.6f} {mw_norm:.6f} {mh_norm:.6f}"
            )
