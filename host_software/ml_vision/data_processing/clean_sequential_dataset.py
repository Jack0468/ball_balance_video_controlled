...
def clean_dataset(data_dir):
    ...
    if "touch_x" in df_cleaned.columns and "touch_y" in df_cleaned.columns:
        ...
        is_frozen = (df_cleaned["touch_x"] == df_cleaned["touch_x"].shift(1)) & (
            df_cleaned["touch_y"] == df_cleaned["touch_y"].shift(1)
        )

        frozen_count = is_frozen.sum()
        df_cleaned = df_cleaned[~is_frozen]
        print(
            f"Removed {frozen_count} frozen telemetry frames (ball off-board or bouncing)."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Deduplicate telemetry to 1 row per image"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
