...
def main():
    parser = argparse.ArgumentParser(
        description="Preprocess synchronized dataset (crop, resize, save)."
    )
    parser.add_argument("--video", required=True, help="Path to raw .MOV video")
    parser.add_argument(
        "--synced-csv",
        required=True,
        help="Path to intermediate synced telemetry mapping",
    )
    parser.add_argument(
        "--output-dir", default="host_software/data/02_silver", help="Output directory"
    )
    parser.add_argument(
        "--crop",
        required=False,
    ...
    preprocessor = DatasetPreprocessor(
        args.video, args.synced_csv, args.output_dir, args.crop
    )
