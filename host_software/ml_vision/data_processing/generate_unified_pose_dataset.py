...
def generate_random_warp(img_w, img_h):
    ...
    pts2 = np.float32(
        [
            ...
            [
                img_w + random.randint(-jitter_x, jitter_x),
                random.randint(-jitter_y, jitter_y),
            ],
            [
                img_w + random.randint(-jitter_x, jitter_x),
                img_h + random.randint(-jitter_x, jitter_x),
            ],
            [
                random.randint(-jitter_x, jitter_x),
                img_h + random.randint(-jitter_x, jitter_x),
            ],
        ]
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="02_silver",
        help="Name of dataset in data/ directory",
    )
    parser.add_argument(
        "--csv_name",
        type=str,
        default="labels_sequential.csv",
        help="Name of the telemetry CSV file",
    )
    args = parser.parse_args()

    script_dir = os.path.dirname(os.path.abspath(__file__))
    input_dir = os.path.abspath(
        os.path.join(script_dir, f"../../data/{args.input_dataset}/images")
    )
    telemetry_path = os.path.abspath(
        os.path.join(script_dir, f"../../data/{args.input_dataset}/{args.csv_name}")
    )

    output_dir = os.path.abspath(
        os.path.join(script_dir, f"../../data/{args.input_dataset}_unified_pose")
    )
    ...
    try:
        pose_model = YOLO(
            os.path.abspath(
                os.path.join(
                    script_dir, "../models/yolov8_platform_pose_v2/weights/best.pt"
                )
            )
        )
    ...
    for idx, img_name in enumerate(image_files):
        ...
        img_warped = cv2.warpPerspective(
            img, H_warp, (w, h), borderMode=cv2.BORDER_REPLICATE
        )
        ...
        out_txt_path = os.path.join(
            out_labels_dir, img_name.replace(".jpg", ".txt").replace(".png", ".txt")
        )
