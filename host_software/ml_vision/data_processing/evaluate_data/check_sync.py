...
def main():
    parser = argparse.ArgumentParser(
        description="Quickly verify video/telemetry sync without full preprocessing."
    )
    parser.add_argument("--video", required=True, help="Path to raw .MOV video")
    parser.add_argument(
        "--synced-csv", required=True, help="Path to synced_telemetry.csv"
    )
    parser.add_argument(
        "--crop", required=True, help="Crop box as 'x1,y1,x2,y2' or 'x,y,w,h'"
    )
    parser.add_argument(
        "--start-idx",
        type=int,
        default=3000,
        help="Frame index to start the 10-second preview (default: 3000)",
    )
    parser.add_argument(
        "--output",
        default="sync_check.mp4",
        help="Output mp4 path (default: sync_check.mp4)",
    )
