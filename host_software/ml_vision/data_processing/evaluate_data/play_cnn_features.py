...
def main():
    parser = argparse.ArgumentParser(description="Play through CNN sequential features to visually verify tracking on raw video.")
    parser.add_argument('--data_dir', default='../../../data/02_silver/session_20260730_174916', help="Path to dataset directory")
    parser.add_argument('--csv_name', default='cnn_sequential_features.csv', help="Name of the CNN features CSV file")
    parser.add_argument('--output', default='validation_labels.mp4', help="Output mp4 path")
    parser.add_argument('--max_frames', type=int, default=1500, help="Max frames to play")
    parser.add_argument('--bronze_dir', default='../../../data/01_bronze/session_20260730_174916', help="Path to original bronze dataset directory")
    ...
    print(f"Generating playback for up to {args.max_frames if args.max_frames != -1 else 'all'} frames...")
    
    frame_idx = 0
    saved_frames = 0
    
    pbar_total = args.max_frames if args.max_frames != -1 else int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    if pbar_total <= 0:
    ...
    while cap.isOpened() and (args.max_frames == -1 or saved_frames < args.max_frames):
        ...
        if ts is not None and ts in features_map:
            ...
            # Annotations
            cv2.putText(frame, "Red: True Telemetry | Blue: CNN Pred", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
            cv2.putText(frame, f"CNN Pixels X:{cnn_px} Y:{cnn_py}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0), 2)
            cv2.putText(frame, f"Touch Pixels X:{touch_px} Y:{touch_py}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
            cv2.putText(frame, f"Video Frame: {frame_idx}", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
