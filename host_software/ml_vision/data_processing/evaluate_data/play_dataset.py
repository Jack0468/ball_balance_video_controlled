...
def main():
    parser = argparse.ArgumentParser(
        description="Play through a cleaned dataset to visually verify telemetry."
    )
    parser.add_argument(
        "--data_dir",
        required=True,
        help="Path to dataset directory (e.g. ../../data/02_silver)",
    )
    parser.add_argument(
        "--csv_name",
    ...
    parser.add_argument(
        "--output",
        default="dataset_playback.mp4",
        help="Output mp4 path (default: dataset_playback.mp4)",
    )
    parser.add_argument(
        "--max_frames",
        type=int,
        default=1500,
        help="Max frames to play (default: 1500, roughly 50s)",
    )
    ...
    print(
        f"Dataset has {len(df)} frames. Generating playback for up to {args.max_frames} frames..."
    )
    ...
    for i in tqdm(range(frames_to_process)):
        ...
        cv2.putText(
            frame,
            f"Dataset Row: {i} | Image: {row['image_file']}",
            (10, 90),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (255, 255, 255),
