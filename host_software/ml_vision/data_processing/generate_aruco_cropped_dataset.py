...
def main():
    ...
    parser.add_argument(
        "--video", required=True, help="Path to rgb_video.mp4 from 01_bronze"
    )
    parser.add_argument(
        "--out_dir",
        required=True,
        help="Directory to save the new dataset (e.g., 02_silver/session_X)",
    )
    ...
    if os.path.exists(telemetry_path) and os.path.exists(timestamps_path):
        ...
        print(
            f"Successfully synced {len(synced_telemetry)} frames with touch pad telemetry!"
        )
    else:
        print(
            "Warning: telemetry.csv or frame_timestamps.csv not found in the video folder. No touch data will be saved."
        )
    ...
    if os.path.exists(csv_path):
        try:
            ...
            if not existing_df.empty:
                ...
                saved_count = (
                    int(last_frame.replace("frame_", "").replace(".jpg", "")) + 1
                )
                print(
                    f"Found existing dataset with {len(existing_df)} entries. Appending starting from frame_{saved_count:04d}.jpg"
                )
    ...
    while True:
        ...
        if current_frame_index in synced_telemetry:
            ...
            if prev_touch_x is not None:
                dist = (
                    (touch_x_centered - prev_touch_x) ** 2
                    + (touch_y_centered - prev_touch_y) ** 2
                ) ** 0.5
