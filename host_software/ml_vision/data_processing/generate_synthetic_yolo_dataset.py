...
def main():
    ...
    output_dir = os.path.abspath(
        os.path.join(script_dir, "../../data/03_synthetic_yolo")
    )
    ...
    print(
        f"Generating {augmentations_per_image} synthetic variations for {len(image_paths)} images..."
    )
    ...
    for img_path in image_paths:
        ...
        for aug_idx in range(augmentations_per_image):
            ...
            for class_id, color_bgr in markers_config:
                ...
                if pt is None:
                    print(
                        f"Warning: Could not find valid spot for class {class_id} in {basename}"
                    )
                    continue
            ...
            with open(
                os.path.join(out_labels_dir, out_name.replace(".jpg", ".txt")), "w"
            ) as f:
                f.write("\n".join(labels) + "\n")
