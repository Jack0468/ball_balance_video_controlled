...
def normalize_dataset(csv_path):
    ...
    majority_freq = (
        int(non_outlier_counts.median()) if not non_outlier_counts.empty else 1
    )
    majority_freq = max(1, majority_freq)  # Ensure at least 1

    print(f"Total rows: {len(df)}")
    print(
        f"Grid cell frequencies - min: {cell_counts.min()}, max: {cell_counts.max()}, median: {int(cell_counts.median())}"
    )
    ...
    print(
        f"Rows after normalization: {len(df_normalized)} (dropped {len(df) - len(df_normalized)})"
    )
