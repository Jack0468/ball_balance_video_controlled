...
class DataSynchronizer:
    ...
    def run_sync(self, output_csv_path):
        ...
        for i in range(total_frames):
            ...
            frame_timestamps.append({"frame_index": i, "pos_msec": pos_msec})
        ...
        left_closer = (indices > 0) & (
            np.abs(video_times - tel_times[indices - 1])
            < np.abs(video_times - tel_times[indices])
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Synchronize telemetry data with video VFR timestamps."
    )
    parser.add_argument("--video", required=True, help="Path to the raw .MOV video")
    parser.add_argument(
        "--telemetry", required=True, help="Path to the raw telemetry .csv"
    )
    parser.add_argument(
        "--sync-frame",
        required=True,
        type=int,
        help="The exact video frame index where the timestamp was read",
    )
    parser.add_argument(
        "--sync-timestamp",
        required=True,
        type=int,
        help="The exact Unix timestamp read from the screen at sync-frame",
    )
    parser.add_argument(
        "--output", required=True, help="Path to output the synced CSV mapping"
    )

    args = parser.parse_args()

    sync = DataSynchronizer(
        args.video, args.telemetry, args.sync_frame, args.sync_timestamp
    )
