...
def redraw():
            ...
            if c_id == 0:
                ...
                if len(parts) >= 17:
                    ...
                    if len(pts) == 4:
                        for i in range(4):
                            cv2.line(
                                display_image, pts[i], pts[(i + 1) % 4], (0, 255, 0), 2
                            )
            elif c_id == 1:
    ...
    cv2.putText(
        display_image,
        "If new frame: Click 4 corners (TL, TR, BR, BL), then 1 for Ball.",
        (20, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (0, 0, 255),
    ...
    cv2.putText(
        display_image,
        "Markers: 2(Blu),3(Gry),4(Blk),5(Red),6(Grn),7(Yel),8(Cya),9(Pur),o(Org),p(Pnk),b(Brn)",
        (20, 60),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (0, 0, 255),
    ...
    cv2.putText(
        display_image,
        "Press 's' to save/next, 'c' to clear new points, 'x' to skip frame.",
        (20, 90),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (0, 0, 255),
...
def main():
    ...
    parser.add_argument(
        "--bronze-dir",
        default="host_software/data/01_bronze",
        help="Dir containing video folders",
    )
    parser.add_argument(
        "--image-dir",
        required=False,
        help="Optional directory with existing JPG/PNG images",
    )
    parser.add_argument(
        "--out-dir",
        default="host_software/data/yolo_raw_dataset",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Frames to extract per video or number of images to sample",
    )
    parser.add_argument(
        "--copy-images",
        action="store_true",
        help="Copy sampled images into output images directory instead of labeling in place",
    )
    ...
    if args.image_dir:
        source_image_paths = sorted(
            ...
            glob.glob(os.path.join(args.image_dir, "**", "*.png"), recursive=True)
        )
        ...
        if not source_image_paths:
            ...
            return
        source_image_paths = random.sample(
            source_image_paths, min(args.frames_per_video, len(source_image_paths))
        )
        if args.copy_images:
            print(
                f"Copying {len(source_image_paths)} sampled images from {args.image_dir}..."
            )
        ...
        if len(existing_images) == 0:
            ...
            video_paths = glob.glob(
                os.path.join(args.bronze_dir, "**", "*.MOV"), recursive=True
            )
            image_paths = sorted(
                glob.glob(os.path.join(args.bronze_dir, "**", "*.jpg"), recursive=True)
            ) + sorted(
                glob.glob(os.path.join(args.bronze_dir, "**", "*.png"), recursive=True)
            )
            ...
            if video_paths:
                ...
                print(
                    f"Sampling {args.frames_per_video} existing images from {args.bronze_dir}..."
                )
            ...
            while True:
                ...
                if key == ord("c"):
                    ...
                    if len(clicked_points) == 5:
                        ...
                        print(
                            "Warning: Incomplete clicks for platform/ball. They won't be saved."
                        )

                    # Process markers
                    for c_id, mx, my in marker_points:
                        m_label = format_yolo_pose(
                            c_id, mx, my, 20.0, 20.0, img_w, img_h, keypoints=None
                        )
