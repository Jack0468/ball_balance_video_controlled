...
def get_aruco_homography(img):
    ...
    M, _ = cv2.findHomography(np.array(phys_pts, dtype=np.float32), np.array(img_pts, dtype=np.float32))
    return M
...
def main():
    ...
    parser.add_argument("--video", default="host_software/data/01_bronze/session_20260730_174916/rgb_video.mp4")
    ...
    while True:
        ...
        if np.any(warped_corners < -100) or np.any(warped_corners[:, 0] > w + 100) or np.any(warped_corners[:, 1] > h + 100):
            continue
