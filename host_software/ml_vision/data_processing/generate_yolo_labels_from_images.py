...
def main():
    parser = argparse.ArgumentParser(
        description="Generate YOLO labels from existing images and labels.csv"
    )
    parser.add_argument(
        "--data_dir",
        required=True,
        help="Path to data directory (e.g., data/02_silver)",
    )
    parser.add_argument(
        "--calibration_image",
        required=False,
        help="Optional: Path to a well-lit image taken from the EXACT same camera angle to use for clicking the 4 markers.",
    )
    ...
    while True:
        ...
        while len(clicked_points) < 4:
            ...
            if key == ord("n"):
                if frame_index == -1:
                    print(
                        "Cannot skip custom calibration image. Please press 'c' to change contrast or close the window."
                    )
                else:
                    skip = True
                    break
            ...
            if frame_index != -1:
                ...
                while frame_index < len(df):
                    next_image_path = os.path.join(
                        images_dir, df.iloc[frame_index]["image_file"]
                    )
