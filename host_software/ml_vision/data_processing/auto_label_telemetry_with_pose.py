...
def main():
    ...
    for session in sessions:
        ...
        for idx, row in tqdm(
            df.iterrows(), total=len(df), desc=f"Auto-labeling {session}"
        ):
            ...
            touchpad_corners_mm = np.array(
                [[0, 0], [TOUCHPAD_W, 0], [TOUCHPAD_W, TOUCHPAD_H], [0, TOUCHPAD_H]],
                dtype=np.float32,
            )

            touchpad_pixel_corners = cv2.perspectiveTransform(
                np.array([touchpad_corners_mm]), M_inv
            )[0]
            ...
            if M_crop is not None:
                ...
                ball_pt_cropped = cv2.perspectiveTransform(
                    np.array([[ball_pt_pixel]]), M_crop
                )[0][0]
