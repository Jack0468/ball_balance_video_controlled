...
def run_pipeline(source=0, headless=False, fallback="raw", max_frames=None):
    ...
    while True:
        ...
        if frame_count % 30 == 0:
            ...
            print(
                f"FPS: {1000/safe_latency:.1f} | Preproc: {preproc_time:.1f}ms | YOLO: {infer_time:.1f}ms | Total: {latency:.1f}ms"
            )
    ...
    if frame_count > 0:
        ...
        print(
            f"Average Preprocessing Latency: {total_preproc_latency / frame_count:.1f}ms"
        )
        print(
            f"Average YOLO Inference Latency: {total_infer_latency / frame_count:.1f}ms"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "source", nargs="?", default=0, help="Webcam ID (default: 0) or video file path"
    )
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Run without rendering cv2 GUI windows for pure latency testing",
    )
    parser.add_argument(
        "--fallback",
        choices=["raw", "skip"],
