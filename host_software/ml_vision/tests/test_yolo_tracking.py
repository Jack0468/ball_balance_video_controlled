...
def test_yolo(source, max_frames=None):
    ...
    def process_frame(frame):
        ...
        if len(boxes) > 0:
            for box in boxes:
                ...
                print(
                    f"Frame {frame_count}: Sports ball detected at ({cx:.1f}, {cy:.1f}) - Latency: {latency:.1f}ms"
                )
        else:
    ...
    if frame_count > 0:
        print(
            f"Average latency: {total_latency / frame_count:.1f}ms ({1000 / (total_latency / frame_count):.1f} FPS)"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "source", help="Webcam ID (e.g. 0), video file path, or image directory"
    )
    parser.add_argument(
        "--max_frames", type=int, default=None, help="Max frames to process"
    )
