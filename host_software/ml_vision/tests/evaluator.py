...
class Evaluator:
    ...
    def visualize_predictions(self, frame, true_pos, pred_pos, frame_idx):
        ...
        if true_pos is not None and not np.isnan(true_pos[0]):
            cv2.circle(
                viz_frame, (int(true_pos[0]), int(true_pos[1])), 8, (0, 255, 0), -1
            )
            cv2.putText(
                viz_frame,
                "True",
                (int(true_pos[0]) + 10, int(true_pos[1])),
        ...
        if pred_pos is not None:
            cv2.circle(
                viz_frame, (int(pred_pos[0]), int(pred_pos[1])), 6, (0, 0, 255), -1
            )
            cv2.putText(
                viz_frame,
                "Pred",
                (int(pred_pos[0]) + 10, int(pred_pos[1]) - 15),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 255),
                2,
    ...
    def calculate_metrics(self, results_dict):
        ...
        for model_name, metrics in results_dict.items():
            ...
            print(
                f"  Valid Predictions: {metrics['valid_predictions']} / {metrics['total_frames']}"
            )
            print("-" * 25)
