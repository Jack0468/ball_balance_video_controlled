    ...
    def run_benchmark(self, frames, labels):
        ...
        for name, model in self.models.items():
            ...
            for frame, true_pos in zip(frames, labels):
                ...
                if (
                    pred is not None
                    and true_pos is not None
                    and not np.isnan(true_pos[0])
                ):
...
if __name__ == "__main__":
    ...
    if not sample_frames:
        print(
            "No images found. Please ensure images exist in the data/image/YT_video1/ folder."
        )
    else:
        sample_labels = [None] * len(
            sample_frames
        )  # Dummy labels since we don't have annotated (Gold) data yet
        ...
        for name, res in results.items():
            ...
            print(
                f"Valid Predictions: {res['valid_predictions']}/{res['total_frames']}"
            )
