    ...
    def get_perspective_transform(self, frame, draw_corners=False):
        ...
        for c in contours:
            ...
            if len(approx) == 4 and cv2.isContourConvex(approx):
                ...
                cv2.drawContours(
                    poly_outline, [approx], -1, 255, 2
                )  # 2 pixel thickness

                overlap = cv2.bitwise_and(poly_outline, mask_edges)
                overlap_ratio = np.count_nonzero(overlap) / (
                    np.count_nonzero(poly_outline) + 1e-6
                )
                ...
                if (
                    avg_stddev > 50.0
                ):  # Adjust this threshold if it rejects the real platform!
                    continue
        ...
        if pts is not None and len(pts) == 4:
            ...
            if self.prev_corners is not None:
                ...
                if dist < 150:  # Threshold for valid EMA smoothing
                    rect = (
                        self.ema_alpha * rect + (1 - self.ema_alpha) * self.prev_corners
                    )
