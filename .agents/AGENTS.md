...
- **Compute (Large-Model Deployment):** NVIDIA **Jetson AGX Orin 64GB Developer Kit model: p3730** (confirmed 2026-08-13 — Ampere GPU w/ Tensor Cores, NOT the original 2019 Maxwell-based Jetson Nano, which is ~15-40x weaker and has no Tensor Cores). Target device for large multimodal/VLA models that are out of scope for both the FPGA (612.5 KB BRAM / 220 DSP budget) and the STM32. Not used for the lightweight expert-side vision/audio/control models. Realistic capability ceiling: Much higher than the previous 8GB limit. OpenVLA (7B) runs at ~2Hz. See Multimodal/VLA section below for what this rules in/out.
...
### Arm 1 (Expert) has two vision model classes, not one (2026-08-14)

Reframed from a binary "which one wins" question into an explicit taxonomy — both classes are legitimate members of Arm 1, not a gate to pick a single winner before proceeding:

- **"Small model" class:** the Shared Backbone CNN (`shared_vision_backbone_v1`, ~64-91K params depending on decoder choice). Built as a future FPGA target (tiny-parameter design), currently in training.
- **"Medium model" class:** the YOLO/ResNet lineage (`yolov8_platform_pose_markers_iphone_v1` + `mlp_corrector_iphone_v1` — what `run_eval_expert.py` actually runs today — plus `cnn_2d_tracker_0730_v3`/BasicCNN ~340K, YOLOv8-nano ~3.2M, ResNet18/50 variants). Now that FPGA vision inference is paused, parameter count is no longer a constraint on using this class — see the lifted-ban note above.

**Both classes aim at the same vision task (ball + marker detection) but were trained on different, merely similar, data — this is the real lurking variable, not architecture size.** The medium-class lineup (tags `_iphone_`, `_0728_`, `_0730_`) was never trained on Dataset 8/9 — the merged, bug-fixed, synthetic-augmented dataset (52,772 real + 59,336 synthetic rows) that only the small-class `shared_vision_backbone_v1` has seen. Comparing the two classes' accuracy as-is conflates "architecture" with "which one saw better data," not isolate either variable cleanly.

**Decision:** deliberately left open — both classes are candidate Arm 1 vision components, pending the small class's evaluation results once its current training run finishes. Do not pick a side (don't assume "the medium class wins because proven" and don't assume "the small class wins because newer data") until that evaluation exists. A clean architecture-only ablation (does model size matter, holding data constant) would require retraining the medium class on Dataset 8/9 first — flagged as a low-priority "for fun" follow-up, not a blocker, and not worth the time investment unless there's spare capacity for it.
...
- `firmware/stm32_ml_control_and_vision/BallBalancingBot/`: **Current source of truth for STM32 firmware, expert-pipeline/laptop-driven arm** (corrected 2026-08-13 — this doc previously pointed at `firmware/BallBalancingBot/`, which is stale; that directory still has the older PID-only, touchscreen-only control loop). The current firmware runs a trained RL actor network (`RLControl.cpp`, weights baked in via `weights_best_so_far1.h`) instead of classical PID, and already accepts ball/target coordinates over serial from an external host via `SerialCoords.cpp` (a drop-in replacement for the touchscreen reader — see its header comment for the wire protocol). Do not add new firmware sketches for data collection.
- `firmware/stm32_jetson_remote_control/BallBalancingBot/`: **Intentional, documented, separate firmware deployment for the Jetson AGX Orin comparison arm** (added 2026-08-19) — do not confuse with the "six undocumented duplicates" flagged below, this one exists on purpose and is explained in its own `BallBalancingBot.ino` header comment. Built from the same shared modules as the current source-of-truth firmware (`MotorControl.cpp/.h`, `InverseKinematics.cpp/.h`, `RemoteStepControl.cpp/.h`, unchanged) plus a modified `TouchProbe.cpp/.h` that drops the `SerialCoords.cpp`/`Screen.h` dependency entirely — `RemoteStepControl.cpp`'s `"S,<a>,<b>,<c>"` line reader and `SerialCoords.cpp`'s downlink parser cannot safely share one `Serial` RX stream in the same `loop()` (both drain `Serial.available()` independently; whichever runs first can silently starve the other of every step command), so this firmware's `loop()` never includes `SerialCoords.cpp` at all rather than trying to make the two coexist. `loop()` calls `remote_step_control_update()` instead of `rl_balance()` — the STM32 executes pre-computed step targets from `host_software/ml_jetson_vla/core/control_net.py` (a NumPy port of the *same* trained policy `RLControl.cpp` runs on-device) running on the Jetson, instead of running inference itself. Working principle going forward: **each deployment/arm gets its own dedicated firmware built from shared modules, not a shared sketch toggled in place** — this is why a new directory was created instead of editing `stm32_ml_control_and_vision/`'s `loop()`.
- **⚠️ FLAG (2026-08-13, not yet resolved):** at least **six** directories contain a `BallBalancingBot.ino`/firmware copy — `firmware/BallBalancingBot/`, `firmware/stm32_ml_control_and_vision/BallBalancingBot/` (current, per above), `firmware/ExpertEvaluationFirmware/`, `host_software/ml_control/stm32_ctrl/BallBalancingBot/`, `ball-balancing-bot/BallBalancingBot/`, `host_software/ml_endtoend/firmware/BallBalancingBot/`. Only the current one has been confirmed; the other five have **not** been individually checked for whether they're dead duplicates, legacy snapshots, or serving some distinct purpose (e.g. `ExpertEvaluationFirmware` may be a deliberately separate build for the expert evaluation harness, not just a stale copy). This already caused one wrong recommendation before being caught. Needs a real audit (like the FPGA docs' current-vs-legacy pass in §5 below) — not done yet, flagging so it doesn't get silently assumed either way.
...
- `host_software/ml_jetson_vla/`: Active (added 2026-08-13) — arm 2 of the 3-arm comparison: a large (billions-of-param) Qwen-derived model, adapted from a lab partner's work, deployed standalone on the Jetson AGX Orin 64GB Developer Kit model: p3730. See `host_software/ml_jetson_vla/docs/ARCHITECTURE.md` for full context and open blockers (model access, deployment pipeline).
...
### Phase 6 — Multimodal / VLA Comparative Study (ACTIVE — **THE PROJECT'S MAJOR GOAL as of 2026-08-14** — owned by `.agents/agent_ml_multimodal.md`)

**Two hardware decisions locked in 2026-08-14, superseding earlier "under investigation" language:**
- **Camera:** USB webcam plugged **directly into the Jetson**. No laptop relay, no FPGA/OV7670 path. Reuses the existing `cv2.VideoCapture`-based capture code verbatim. (Laptop→network→Jetson streaming and OV7670-direct-to-Jetson were both considered and rejected — the former risks corrupting the RL control net's timing-sensitive velocity filter with network jitter, especially over WiFi; the latter hits the same OS-scheduling-jitter problem as driving steppers from Jetson, since the OV7670 needs timing-critical parallel-bus capture with no native USB/CSI interface.)
- **Motor actuation:** **STM32, via the existing `SerialCoords.cpp` serial interface, for now.** FPGA Option B (`stepper_motor_controller.v` ported to a Zynq AXI-Lite IP) is deferred, not abandoned — pick it up only if a real project constraint later forces it. Do not proactively build Option B under current priorities.

**The project's major goal right now: evaluate small, medium, and large models all running on the Jetson AGX Orin 64GB Developer Kit model: p3730, and compare them.** This is explicitly the top priority — ahead of the FPGA/photonic track (Phase 5, still real and documented, but not the current focus). Three model classes, one shared hardware platform (controls for hardware as a confound):
  1. **Small class** — Shared Backbone CNN (~64-91K params, FPGA-target design, currently training) for vision, plus the audio classifier and the RL-trained control net (9→32→32→3 MLP, `RLControl.cpp`/`weights_best_so_far1.h` — I/O contract: 9-dim observation in [ball xy, xy error, filtered xy velocity, actual step positions ×3], raw step-space targets out, no angle conversion needed).
  2. **Medium class** — the YOLO/ResNet lineage (`yolov8_platform_pose_markers_iphone_v1` + `mlp_corrector_iphone_v1`, what `run_eval_expert.py` actually runs today; also `cnn_2d_tracker_0730_v3`/BasicCNN, YOLOv8-nano, ResNet18/50 variants), same audio + control net. Both classes aim at the same task on different-but-similar training data — see the "Arm 1 has two vision model classes" callout in §"ML Component Sizes" above.
  3. **Large class** — the billions-of-param Qwen-derived model adapted from the lab partner's work. **Still blocked**: not yet in our possession.
- **Photonic arm (Phase 5) is documented and flagged but explicitly not the current priority** — don't let it compete for attention with the Jetson comparison goal above.
- `host_software/ml_multimodal/`'s existing scaffold (dataset synthesis → Stage 1 BC → Stage 2 RL → evaluation) still applies to `RT1LiteVLA` as a secondary baseline; several pieces are stubbed/mocked (dummy images, mock marker coords) — audit before trusting comparative numbers from it.
- `host_software/evaluations/evaluate_system_control.py` is the shared comparison tool across all model classes (`--runs label=csv label=csv ...`) — fixed 2026-08-13 to normalize timestamp-column differences across schemas and to match the documented metric definitions. This is the tool the major goal above needs eval runs feeding into.
- Related external context: a lab partner is building a comparable Qwen-based model for the same task, which the large class adapts from — see "External / Comparative Context" above.
